...
from data_utils import Dataset_ASVspoof2019_eval
...
#    prefix_2021 = 'ASVspoof2021.{}'.format(track)
    ...
    #遍历模型的每个参数，计算每个参数的大小，比如其中一个参数大小是最后化成一维整数，所以
    #最后相加是模型总的参数量
    ...
    if args.eval:
        ...
        file_eval = genSpoof_list(dir_meta=os.path.join(
            args.protocols_path + '{}_cm_protocols/{}.cm.eval.trl.txt'.format(prefix, prefix_2019)), is_train=False,
                                  is_eval=True)
...
#        eval_set=Dataset_ASVspoof2021_eval(list_IDs = file_eval,base_dir = os.path.join(args.database_path+'ASVspoof2021_{}_eval/'.format(args.track)))
        eval_set = Dataset_ASVspoof2019_eval(list_IDs=file_eval, base_dir=os.path.join(
            args.database_path + 'ASVspoof2019_{}_eval/'.format(args.track)))
        #奇怪，测试集没有数据加载器吗？交叉集计算精度都有
        produce_evaluation_file(eval_set, model, device, args.eval_output)
        sys.exit(0)

     
    # define train dataloader
    #对比genSoof_list,训练和测试，前者多返回d_label_trn，后者无
